...
```{r}
d_eeg %>% group_by(subject) %>% summarise(count = n()) %>% knitr::kable()
```
...
```{r}
d_dispthresh %>% group_by(subject) %>% summarise(count = n()) %>% knitr::kable()
```
...
We will remove identity relationships (i.e., a group is a subgroup of itself) and the three pairs of wallpapers groups that it subgroups of each other (e.g., PM is a subgroup of CM, and CM is a subgroup of PM). This leaves us with a total of `r nrow(subgroups)` subgroups to include in our analysis.
...
```{r, fig.height=8, fig.cap="Distributions of the difference in mean log(rms) between sub- and super-groups. The index of each relationship is indicated by the colour of the y-axis label. The fill of the density plots indicated the probability of the difference being greater than zero.", cache=TRUE}
...
```{r ,fig.height=8, cache = TRUE, fig.cap="Distributions of the difference in mean log display duration threshold between sub- and super-groups. The index of each relationship is indicated by the colour of the y-axis label. The fill of the density plots indicated the probability of the difference being less than zero."}
...
```{r fig.height=8, cache=TRUE, echo = FALSE, fig.cap="Distributions of the difference in mean log(rms) between sub- and super-groups. The index of each relationship is indicated by the colour of the y-axis label. The fill of the density plots indicated the probability of the difference being greater than zero."}
...
df %>% gather("measurement", "number", -p) %>%
  ...
  geom_path(size = 1.5) +
  theme_minimal() + 
  ggthemes::scale_color_ptol(labels = c("primary EEG", "sensor control EEG", "harmonic control EEG", "psychophysical threshold")) +
...
We can look at the groups that only contain orientation, and see if we obtain the parametric response as documented in Kohler et al. (2016). We will also investigate if we see the corresponding pattern with the display duration threshold data, with the time taken to detect the symmetry dreasing as we increase the amount of rotational symmetry.


```{r, cache = TRUE}
d_eeg %>% filter(wg %in% c("P2", "P3", "P4", "P6")) %>%
  mutate(orientation = parse_number(wg))-> d_eeg_rot 
...
d_eeg_rot %>% modelr::data_grid(orientation) %>%
  ...
  ggplot() + 
  geom_line(data = d_eeg_rot, aes(x = orientation, y = rms, group = subject), alpha = 0.1, colour = "darkred") +
  geom_path(aes(x = orientation, y = .value, group = .draw), alpha = .1, colour = "royalblue2") + 
  theme_minimal() -> p1
```
```{r, cache = TRUE}
d_dispthresh %>% filter(wg %in% c("P2", "P3", "P4", "P6")) %>%
  mutate(orientation = parse_number(wg))-> d_disp_rot 

m_disp_rot <- brm(data = d_disp_rot, threshold ~ orientation + (1|subject), family = "lognormal")

summary(m_disp_rot)

d_disp_rot %>% modelr::data_grid(orientation) %>%
  add_fitted_draws(m_disp_rot, re_formula = NA, n=200) %>%
  ggplot() + 
  geom_line(data = d_disp_rot, aes(x = orientation, y = threshold, group = subject), alpha = 0.1, colour = "darkred") +
  geom_path(aes(x = orientation, y = .value, group = .draw), alpha = .1, colour = "royalblue2") + 
  theme_minimal() -> p2
```
...
comp_summary$eeg %>% ungroup() %>%
   mutate(
     ...
     normal = if_else(normal, 1, 0)) -> comp_summary$eeg
...
comp_summary$threshold %>% ungroup() %>%
   mutate(
     index = as.numeric(as.character(index)),
     normal = if_else(normal, 1, 0)) -> comp_summary$threshold
