...
d_eeg %>% filter(wg %in% c("P2", "P3", "P4", "P6")) %>% ggplot(aes(x = wg, y = log(rms))) + geom_boxplot(notch = T)
   
d_eeg %>% group_by(wg) %>% summarise(
  mean_log_rms = mean(log(rms)),
  sd_log_rms = sd(log(rms)),
  median_log_rms = median(log(rms)))                                                               
...
```{r, fig.cap = "The effect of increasing the amount of rotational symmetry.", fig.height=3}
my_samples$eeg %>% select(P2, P3, P4, P6) %>%
  pivot_longer(names_to = "wallpaper group", cols = c(P2, P3, P4, P6)) %>% 
  mutate(exp = "eeg") %>%
  bind_rows(
    my_samples$threshold %>% select(P2, P3, P4, P6) %>% 
      pivot_longer(names_to = "wallpaper group", cols = c(P2, P3, P4, P6)) %>%
      mutate(exp = "display thresholds")) %>% 
  ggplot() + aes(x = value, fill = `wallpaper group`) + 
...
d_eeg_rot %>% modelr::data_grid(orientation) %>%
  ...
  geom_path(aes(x = orientation, y = .value, group = .draw), alpha = .2, colour = "thistle4") + 
  theme_minimal() 
```
